...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if user_form.is_valid() and profile_form.is_valid():
            ...
            profile.user = user
            #And check and save the picture
            if 'picture' in request.FILES:
                 profile.picture = request.FILES('picture')
            profile.save()
            registered = True

            login(request, user=authenticate(username=user.username, password=user.password))
    else:
        user_form = UserForm()
        profile_form = UserProfileForm()
    return render(request, 'rango/register.html', {'user_form' : user_form, 'profile_form' : UserProfileForm, 'registered' : registered}
)
